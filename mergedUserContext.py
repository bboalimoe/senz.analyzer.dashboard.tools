...
table_name_dest='MergedUserContext'
DBTable = Object.extend(table_name_dest)
field_list = ['motion','sound','startTime','endTime','event','prob','location','user'] # omit UserBehavior field
for user,userData in userContextDataDict.items():
    # print 'the length of this userData is %s ' %str(len(userData))
    # print user.id
    # if userData:
    #     print userData[0].get('startTime')
    ...
    for index ,sorted_user_data in enumerate(sorted_user_data_list):
        dbTable = DBTable()
        dbTable.set('user_context',sorted_user_data)
